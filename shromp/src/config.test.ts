...
`;
			...
			assert.equal(config.sourceFolder(), '../shromp-source/');
			assert.equal(config.outputFolder(), '../shromp-public/');
			assert.equal(config.templatesFolder(), 'shromp-theme/templates');
			assert.equal(config.siteAssetsFolder(), 'shromp-theme/assets');
	...
	it('uses right defaults', async () => {
		...
		assert.equal(config.sourceFolder(), '../docs/');
		assert.equal(config.outputFolder(), '../public/');
		assert.equal(config.templatesFolder(), 'default-theme/templates');
		assert.equal(config.siteAssetsFolder(), 'default-theme/assets');
	...
	it('joins paths correctly for path configs', async () => {
		...
		assert.equal(config.templatesFolder("this", "should", "work"), "default-theme/templates/this/should/work");
	});
});
